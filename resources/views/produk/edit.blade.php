...
<div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
    <div class="w-full mb-1">
        <div class="mb-4">
            ...
            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Ubah Produk</h1>
        </div>
    </div>
</div>
...
<div class="flex flex-col">
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <form method="POST" action="{{ route('produk.update', $produk->id ) }}" class="max-w-sm mx-auto my-5">
            @csrf
            @method('PUT')
            <div class="mb-5">
                <label class="block mb-2 text-sm font-medium text-gray-900">Nama Produk</label>
                <input type="text" value="{{ $produk->nama_produk }}" name="nama_produk" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-800 focus:border-gray-800 block w-full p-2.5" required max="255"/>
            </div>
            <div class="mb-5">
                <label class="block mb-2 text-sm font-medium text-gray-900">Harga</label>
                <input type="number" value="{{ $produk->harga }}" name="harga" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-800 focus:border-gray-800 block w-full p-2.5" required min="1"/>
            </div>
            <div class="mb-5">
                ...
                <select class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" name="kategori_id" required>
                    <option value="{{ $produk->kategori_id }}" selected>{{ $produk->kategori->nama_kategori }}</option>
            ...
            <div class="mb-5">
                ...
                <select class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" name="status_id" required>
                    <option value="{{ $produk->status_id }}" selected>{{ $produk->status->nama_status }}</option>
